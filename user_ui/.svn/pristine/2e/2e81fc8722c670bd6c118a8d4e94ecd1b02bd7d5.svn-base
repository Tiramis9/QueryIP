...
func (a *GameOG) QueryRecord(req map[string]interface{}) (interface{}, error) {
	...
	_, ok := req["start_time"]
	if !ok {
		return "", errors.New("start_time invaild")
	}
	_, ok = req["end_time"]
	if !ok {
		return "", errors.New("end_time time invaild")
	}
	data["Key"] = a.Key
	data["Operator"] = a.Operator
	data["SDate"] = req["start_time"]
	data["EDate"] = req["end_time"]
	res, err := HttpPOSTInquire(url, data)
	if err != nil {
		return nil, err
	}
	if len(res) > 0 {
