...
func IPaddrOperate(db *gorm.DB, mid int, ip string, action string) error {
	if action == "block" {
		return db.Table("black_ip_list").Create(RestrictAccess{IP: ip, Status: 2, MerchantId: mid, CreateTime: time.Now().Unix()}).Error
	}
	return db.Table("black_ip_list").Where("merchant_id = ? AND ip = ?", mid, ip).Delete(RestrictAccess{}).Error
}

//添加访问来源地区黑名单
func SourceAreaOperate(db *gorm.DB, mid int, area, action string) error {
	if action == "block" {
		return db.Table("black_ip_list").Create(RestrictAccess{Area: area, Status: 2, MerchantId: mid, CreateTime: time.Now().Unix()}).Error
	}

	return db.Table("black_ip_list").Where("merchant_id = ? AND area = ? ", mid, area).Delete(RestrictAccess{}).Error
}
