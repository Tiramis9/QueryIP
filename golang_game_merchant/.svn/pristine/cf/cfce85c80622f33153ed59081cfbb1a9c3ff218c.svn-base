...
func checkEditeGetIdParam(msg map[string]interface{}) (int, error) {
	//判断类型 取ID信息
	var id int
...
//修改商户活动
func MerchantActiveEdit(c *gin.Context) {
	...
	aInfoId, err := checkEditeGetIdParam(m)
	if err != nil {
		RespParamErr(c)
		logrus.Println(err)
		return
	...
	//验证成功
	id, err := ma.EditMerchantActivePay(model.Db)
	if err != nil {
		RespServerErr(c)
		return
	}
