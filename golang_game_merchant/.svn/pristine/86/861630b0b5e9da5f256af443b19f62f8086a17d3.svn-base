...
func RouteInit() *gin.Engine {
	...
	v1 := engine.Group("/" + Version)
	{
		...
		v1.POST("/query_basic_info", action.QueryMerchantBasicInfo)              //1-三级基本信息
		v1.POST("/modify_basic_info", action.ModifyMerchantBasicInfo)            //  编辑基本信息
		v1.POST("/query_member_registration", action.QueryMemberRegistration)    //2-会员注册页面设置(查询)
		v1.POST("/modify_member_registration", action.ModifyMemberRegistration)  //  编辑注册页面设置(修改)
		v1.POST("/query_merchant_game_status", action.QueryMerchantGameStatus)   //3-商户游戏状态
		v1.POST("/modify_merchant_game_status", action.ModifyMerchantGameStatus) //  商户游戏状态编辑
		v1.POST("/block_ip_address", action.BlockIPAddr)                         //4-屏蔽ip地址
		...
		v1.POST("/query_warning_log", action.QueryWarningLog)                    //5-预警日志查询
		v1.POST("/query_warning_policy", action.QueryWarningPolicy)              //6-商户预警策略查询
		v1.POST("/query_sys_warning_policy", action.QuerySysWarningPolicy)       //  系统预警策略查询
		v1.POST("/modify_warning_policy", action.ModifyWarningPolicy)            //  商户预警策略修改
		v1.POST("/add_warning_policy", action.AddWarningPolicy)                  //  商户预警策略增加
		v1.POST("/del_warning_policy", action.DelWarningPolicy)                  //  商户预警策略删除
		v1.POST("/query_employee_info", action.QueryEmployeeInfo)                //7-查询员工信息
		...
		v1.POST("/query_employee_role", action.QueryEmployeeRole)                //8-查询员工信息
		...
		v1.POST("/operation_log", action.QueryOperationLog)                      //9-操作日志
		...
		v1.POST("/agent_bank_list", action.AgentBankList) //代理银行
		...
		v1.POST("/agent_class_add", action.AgentClassAdd)                   //代理层级新增
		...
		v1.POST("/merchant_active_edit", action.MerchantActiveEdit)         // 商户编辑活动
