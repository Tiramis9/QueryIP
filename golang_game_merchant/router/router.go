...
func RouteInit() *gin.Engine {
	...
	v1 := engine.Group("/" + Version)
	{
		...
		v1.POST("/shield_ip_address", action.ShieldIpAddress)                           //屏蔽ip地址
		v1.POST("/shield_area_address", action.ShieldAreaAddress)                       //屏蔽地区
		...
		v1.POST("/member_bill", action.MemberBill)                                      //财务管理(帐变记录)
		v1.POST("/recharge_bill", action.RechargeBill)                                  //财务管理(充值记录)
		v1.POST("/recharge_trans_bill", action.RechargeTransBill)                       //财务管理帐变记录(转账)
		v1.POST("/withdraw_bill", action.WithdrawBill)                                  //财务管理(提现记录会员提现)
	...
	v1Auth := v1.Use(auth.Auth)
	{
		...
		/*v1Auth.POST("/logout",action.UserLogout)*/
	}
